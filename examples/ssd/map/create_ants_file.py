...
def main(args):
    ...
    for class_name in classes:
        class_dict[class_name] = open(os.path.join(args.output, 'gt_' + class_name + '.txt'), 'w')
        ...
        for item in itemlist:
            ...
            for bb in bndbox:
                ...
                if name in class_dict:
                    class_dict[name].write(str(os.path.splitext(os.path.basename(f))[0]) +
                            ' ' + (xmin.split('.')[0]) + ' ' + (ymin.split('.')[0]) +
                            ' ' + (xmax.split('.')[0]) + ' ' + (ymax.split('.')[0]) + '\n')
